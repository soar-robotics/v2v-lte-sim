...
if appParams.nRSUs>0 && ~strcmpi(appParams.RSU_pckTypeString,'CAM')
    timeManagement.generationInterval(end-appParams.nRSUs+1:end) = 0.05;
end
%timeManagement.generationInterval(stationManagement.activeIDsLTE) = appParams.averageTbeacon;
timeManagement.timeNextPacket = Inf * ones(simValues.maxID,1);
timeManagement.timeNextPacket(stationManagement.activeIDs) = timeManagement.generationInterval(stationManagement.activeIDs) .* rand(length(stationManagement.activeIDs),1);
...
%if simParams.technology ~= 2 % not only 11p
   ...
   if simParams.BRAlgorithm==2 || simParams.BRAlgorithm==7 || simParams.BRAlgorithm==10
    ...
    if simParams.BRAlgorithm==18
        ...
        %timeManagement.timeOfResourceAllocationLTE(stationManagement.activeIDsLTE) = timeManagement.timeNextPacket(stationManagement.activeIDsLTE);

        % Initialize reselection counter (BRAlgorithm=18)
        stationManagement.resReselectionCounterLTE = Inf*ones(simValues.maxID,1);
        stationManagement.resReselectionCounterLTE(stationManagement.activeIDs) = (simParams.minRandValueMode4-1) + randi((simParams.maxRandValueMode4-simParams.minRandValueMode4)+1,1,length(stationManagement.activeIDs));
